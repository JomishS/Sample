...
func ApplyWhereCondition(db *gorm.DB, query string, val interface{}, operation constant.QueryOperation) *gorm.DB {
	...
	fmt.Println("inside applywherecondition")
	switch operation {
...
func ColumnValCheck(columnName constant.ColumnName) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		return db.Where(fmt.Sprintf("%s is null", columnName))
	}
}
