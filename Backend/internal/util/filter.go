...
func ParseFilters(filters string) (filterMap dto.SearchFilters, err error) {
	...
	filterArray := strings.Split(filters, constant.CommaDelimiter)
	len:=len(filterArray)
	for i=0;i<len;i++{
		if(strings.Contains(filterArray[i],constant.SearchOperatorDelimiter)){
	...
	for _, filter := range filterArr {
		...
		// fmt.Println(filterMap["root"]["in"]["author"])
		fmt.Println(filterMap[table][operator][column])
		i++
	}
